...
, house_bill_item_summary AS (
  SELECT
		...
		, total_sku_quanty * value_per_unit * duty_rate AS total_sku_duties
		, total_sku_quanty * value_per_unit * china_duty_rate AS total_china_sku_duties
...
, allocations AS (
	SELECT
		...
		, SAFE_DIVIDE(total_china_sku_duties, SUM(total_china_sku_duties) OVER (PARTITION BY house_bill_number)) AS allocation_percentage_by_china_duties
...
SELECT
	...
	, tariffs.total_tariff_duty * items.allocation_percentage_by_china_duties AS allocated_china_duties
	, tariffs.total_tariff_cost * items.allocation_percentage_by_china_duties AS allocated_china_tariff_cost
